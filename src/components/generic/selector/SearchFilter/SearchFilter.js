...
class SearchFilter extends React.Component {
    ...
    if (searchValue === '') {
      ...
      if (!this.props.category) {
        list = this.leafs && this.leafs
          .filter(element => RegExp(searchValue.toUpperCase()).test(element.name.toUpperCase())).sort(this.compare);
  ...
  makeCategorySearchList = (value) => {
    ...
    for (let i = 0; i < this.parents.length; i++) {
      ...
      const parentMatch = RegExp(value.toUpperCase()).test(element.name.toUpperCase())
        && this.getCheckboxFromId(element.id, this.props.items);
      const children = element.children.filter((element => RegExp(value.toUpperCase()).test(element.name.toUpperCase())));
  ...
  */
   ...
   */
  compare = (a, b) => {
    let nameA = a.name.toUpperCase(); //Changed from label to name
    let nameB = b.name.toUpperCase();//Changed from label to name
    if (nameA < nameB) {
      return -1;
    }
    if (nameA > nameB) {
